    ...
    for all_id, img_labels in tqdm(data_gt.items()):
        ...
        for i in range(len(img_data.crops)):
                ...
                for matched_template in matched_templates:
                    ...
                    try:
                        with torch.no_grad():
                            ...
                            image1_batch, image1_pil, _ = img_utils.preprocess_norm(img_data.ori_crops[i].convert("RGB"),
                                                                                    model_type='mast3r',
                                                                                    load_size=load_size)
            ...
            if config['debug_imgs'] and i % config['debug_imgs'] == 0:
                dbg_img = vis_utils.create_debug_image_only(R_best, t_best,
                                                       np.asarray(img_data.img.convert("RGB")), img_data.cam_K,
