...
def show_correspondences(image1: Image.Image, image2: Image.Image, points1: list, points2: list):
    ...
    # Get width of the first image to offset the points in the second image
    offset = img1.shape[1]
...
# Setup logging
logging.basicConfig(level=logging.INFO, filename="pose_estimation_check.log",
...
if __name__ == "__main__":
    ...
    if config['debug_imgs']:
        debug_img_path = os.path.join("./debug_imgs_fastsam_pandaL2", config['results_file'].split(".csv")[0])
    ...
    extractor_point = PoseViTExtractor(model_type='dinov2_vits14', stride=14, device=device)
        ...
        for obj_index, img_label in enumerate(img_labels):
            ...
            if bbox_gt != [-1, -1, -1, -1]:
                ...
                try:
                    ...
                    img_prep, img_crop, _ = extractor.preprocess(Image.fromarray(img_crop), load_size=(420, 420))
        ...
        for i in range(len(img_data.crops)):
            ...
            if img_data.crops[i] is not None:
                try:
                    ...
                    # matched_templates = utils.find_template_patch(img_data.descs[i],
                    #                                             templates_desc[object_id],
                    #                                             use_bin=False,
                    #                                             use_best_buddies=True,
                    #                                             num_pairs=10,
                    #                                             top_k=config['num_matched_templates'],
                    #                                             )
                ...
                for matched_template in matched_templates:
                    ...
                    try:
                        with torch.no_grad():
                            ...
                            points1, points2, crop_pil, template_pil = extractor_point.find_correspondences(
                                img_data.crops[i],
                                template,
                                layer=9,
                                facet='token',
                            ...
                            _, img_crop, _ = extractor.preprocess(img_data.crops[i], load_size=load_size)
                            _, template_crop, _ = extractor.preprocess(template, load_size=load_size)
                    ...
                    if R_est is None:
                        ...
                        t_est = np.array(templates_gt_new[object_id][matched_template[1]]['cam_t_m2c'])
