            ...
            if img_data.crops[i] is not None:
                ...
                for matched_template in matched_templates:
                    ...
                    try:
                        with torch.no_grad():
                           ...
                           #  confs = torch.minimum(pred1['desc_conf'], pred2['desc_conf']).permute(1, 2, 0)
                            ...
                            # weight_scores = match_scores * np.maximum(masked_scores, 0.1)
                            weight_scores = match_scores

                            w_ori = img_data.crops[i].size[0]
                            h_ori = img_data.crops[i].size[1]

                            # w_t_ori = template.size[1]
                            # h_t_ori = template.size[0]
                    ...
                    try:
                        ...
                        # R_est, t_est, quality = utils.get_pose_from_correspondences_mask(
                        #     points1,
                        #     points2,
                        #     img_data.y_offsets[i],
                        #     img_data.x_offsets[i],
                    ...
                    with open(csv_file_all, mode='a', newline='') as csvfile_all:
                        ...
                        csv_writer.writerow(
                            [img_data.scene_id, img_data.img_name, object_id,
                             weight_scores.sum().numpy(), quality,
                             R_str, t_str,
...
if __name__ == "__main__":
    ...
    headers_all = ['scene_id', 'im_id', 'obj_id', 'score', 'pnp_inliners', 'R', 't', 'time', 'instance_id']
