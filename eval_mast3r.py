...
def process_images(data_gt, templates_gt_new, config, matcher, norm_factors, csv_file, csv_file_all):
    ...
    # Create an elegant counter using itertools.count()
    ...
    for all_id, img_labels in tqdm(data_gt.items()):
        ...
        for obj_index, img_label in enumerate(img_labels):
            ...
            try:
                ...
                img_crop, y_offset, x_offset = img_utils.make_quadratic_crop_ratio(np.array(img), bbox, patch_size=16,
                                                                                   final_ratio=1.0)
        ...
        for i in range(len(img_data.crops)):
            ...
            # Automatically get the next instance_id
                ...
                for matched_template in matched_templates:
                    ...
                    try:
                        with torch.no_grad():
                            ...
                            image1_batch, image1_pil, _ = img_utils.preprocess_norm(img_data.ori_crops[i],
                                                                                    model_type='mast3r',
                                                                                    load_size=load_size)
                            image2_batch, image2_pil, _ = img_utils.preprocess_norm(template, model_type='mast3r',
                                                                                    load_size=load_size)

                            images = load_images_crop_resize([image1_pil, image2_pil], (320, 320), verbose=False)
                           ...
                           #  confs = torch.minimum(pred1['desc_conf'], pred2['desc_conf']).permute(1, 2, 0)

                            # # Process descriptors
                            desc1 = pred1['desc'].squeeze(0).detach()
                            desc2 = pred2['desc'].squeeze(0).detach()

                            # desc1 = pred1['desc'].squeeze(0).detach() * mask1
                            # desc2 = pred2['desc'].squeeze(0).detach() * mask2

                            # desc1 = pred1['desc'].squeeze(0).detach() * confs
                            # desc2 = pred2['desc'].squeeze(0).detach() * confs

                            desc1 = torch.nn.functional.normalize(desc1, p=2, dim=2)
                            desc2 = torch.nn.functional.normalize(desc2, p=2, dim=2)
                            ...
                            #     matcbhes_confs = matches_confs[mask]
                            ...
                            if len(masked_scores) > 0:
                                ...
                                # matches_confs = matches_confs[masked_scores==1]

                            # Apply Score
                            # if len(match_scores) > 0:
                            #     matches_im0 = matches_im0[match_scores>=0.5]
                            #     matches_im1 = matches_im1[match_scores>=0.5]
                            #     match_scores = match_scores[match_scores>=0.5]

                            weight_scores = match_scores
                            confidence_scores = matches_confs
                            ...
                            # match_idx_to_viz = np.round(np.linspace(0, num_matches - 1, n_viz)).astype(int)
                            # matches_im0, matches_im1 = matches_im0[match_idx_to_viz], matches_im1[match_idx_to_viz]
                            # weight_scores = weight_scores[match_idx_to_viz]

                            w_ori = img_data.crops[i].size[0]
                            h_ori = img_data.crops[i].size[1]

                            w_t_ori = img_data.crops[i].size[0]
                            h_t_ori = img_data.crops[i].size[1]

                            points1 = np.round(matches_im0 / np.array([W0 / w_ori, H0 / h_ori])).astype(int)
                            points2 = np.round(matches_im1 / np.array([W0 / w_t_ori, H0 / h_t_ori])).astype(int)

                            # # Use RANSAC to compute the fundamental matrix
                            # fundamental_matrix, mask = cv2.findFundamentalMat(np.array(points1), np.array(points2), cv2.FM_RANSAC)

                            # # Select inlier points (points that were used to compute the fundamental matrix)
                            # points1 = points1[mask.ravel() == 1]
                            # points2 = points2[mask.ravel() == 1]

                            # show_correspondences(image1_pil, image2_pil.resize(load_size), points1, points2)
                    ...
                    try:
                        ...
                        # load_size = (template.size[1], template.size[0])
                        ...
                        weight_scores = weight_scores[inliers].numpy().sum()
                        confidence_scores = confidence_scores[inliers].sum()
                        ...
                        weight_scores = -np.inf
                        confidence_scores = -np.inf


                    # Fallback for pose estimation
                    if R_est is None:
                        R_est = np.array(matched_template['cam_R_m2c']).reshape((3, 3))
                        t_est = np.array(matched_template['cam_t_m2c'])
                        quality = -np.inf
                    ...
                    # err = GT_err
                    err = - weight_scores
                    # err = - confidence_scores

                    # --------------------------------------------------------------------------------------------#
                    # Score the results for each template.
                    # Prepare fower writing results
                    ...
                    with open(csv_file_all, mode='a', newline='') as csvfile_all:
                        csv_writer = csv.writer(csvfile_all)
                        csv_writer.writerow(
                            [img_data.scene_id, img_data.img_name, object_id,
                             GT_err, weight_scores, confidence_scores, quality,
                             R_str, t_str,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_file',
                        default="./database/zs6d_configs/bop_eval_configs/cfg_%s_inference_bop_zero.json"%(dataset_name))
    ...
    headers_all = ['scene_id', 'im_id', 'obj_id', 'GT_error', 'score', 'conf', 'pnp_inliners', 'R', 't', 'time', 'instance_id']
    prepare_csv_file(csv_file_all, headers_all)

    # Prepare debug image path if needed
    if config['debug_imgs']:
        debug_img_path = os.path.join("./logs/debug_imgs_fastsam_panda/"+dataset_name, config['results_file'].split(".csv")[0])
